...
class relatedfree extends Module
{
    ...
    public function getProductDetails(
        ...
        $sql = 'SELECT p.*, product_shop.*, pl.* , m.`name` AS manufacturer_name, s.`name` AS supplier_name
                ...
                '.(!empty($id_attribute_group) ? 'LEFT JOIN ' . _DB_PREFIX_ . 'attribute psa ON psa.id_attribute=pac.id_attribute' : '').'
                WHERE pl.`id_lang` = ' . (int) $id_lang .
                    ($id_category ? ' AND c.`id_category` = ' . (int) $id_category : '') .
                    ($front ? ' AND product_shop.`visibility` IN ("both", "catalog")' : '') .
                    ($only_active ? ' AND product_shop.`active` = 1' : '') . '
                     AND p.id_product='.$id_product.'
                 '.(!empty($id_attribute) ? 'AND pac.id_attribute=' . (int) $id_attribute.' ' : '').' 
                 '.(!empty($id_attribute_group) ? 'AND psa.id_attribute_group= ' . (int) $id_attribute_group.'' : '' ).'
                 group by p.id_product ORDER BY ' . (isset($order_by_prefix) ? pSQL($order_by_prefix) . '.' : '') . '`' . pSQL($order_by) . '` ' . pSQL($order_way) .
    ...
    public function getFilteredProducts(
        ...
        $sql = 'SELECT p.*, product_shop.*, pl.* , m.`name` AS manufacturer_name, s.`name` AS supplier_name
                ...
                WHERE pl.`id_lang` = ' . (int) $id_lang .
                    ...
                    ($only_active ? ' AND product_shop.`active` = 1' : '') . '
                     AND p.id_product!='.$id_product.'
                 '.(!empty($attributes_string) ? 'AND pac.id_attribute in (' . $attributes_string.') ' : '').'
                 group by p.id_product ORDER BY ' . (isset($order_by_prefix) ? pSQL($order_by_prefix) . '.' : '') . '`' . pSQL($order_by) . '` ' . pSQL($order_way) .
                ($limit > 0 ? ' LIMIT ' . (int) $start . ',' . (int) $limit : '');
    ...
    public function hookProductFooter($params)
    {
        ...
        $Context=Context::getContext();
        $current_product = $this->getProductDetails($Context->language->id,0, 1, 'id_product', 'asc',false ,Tools::getValue('id_product'));
        $products_full = Product::getProductsProperties($Context->language->id, $current_product);
        $current_product_properties=current($products_full);
        $id_default_category=$current_product_properties['id_category_default'];
        $current_attributes=$current_product_properties['attributes'];
        $attributes_ids=array();
        foreach ($current_attributes as $key => $value) {
            $attributes_ids[]=$value['id_attribute']; 
        }
        $attributes_string=implode(',',$attributes_ids);
        $blocks_products=$this->getFilteredProducts($Context->language->id,0, 0, 'id_product', 'asc',$id_default_category ,Tools::getValue('id_product'),$attributes_string);
